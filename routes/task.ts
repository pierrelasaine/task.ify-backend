...
taskRoute.post('/', async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({
            error: 'Failed to fetch tasks from database /tasks'
        })
    }
})
...
taskRoute.get('/:id', async (req: Request, res: Response) => {
    try {
        ...
        console.error(error.message)
        res.status(500).json({
            error: 'Failed to fetch task from database /tasks/:id'
        })
    }
})
...
taskRoute.delete('/delete/:id', async (req: Request, res: Response) => {
    try {
        ...
        console.error(error.message)
        res.status(500).json({
            error: 'Failed to delete task in database /tasks/:id'
        })
    }
})

taskRoute.get('/:id/playlistcover', async (req: Request, res: Response) => {
    try {
        const playlistId = req.params.id
