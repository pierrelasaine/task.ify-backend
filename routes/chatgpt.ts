...
gptRoute.post('/generateplaylist', async (req: Request, res: Response) => {
    try {
        ...
        console.log("TASKNAME PLEASEEEEEEE", taskName)
        ...
        const playlistResponse: {data: { playlistId: string, spotifyId: string }} = await axios.post('http://localhost:3001/playlist/callback', { playlistName, tracks, accessToken });

        const playlistId = playlistResponse.data.playlistId;
        const spotifyId  = playlistResponse.data.spotifyId;

        console.log("BEFORE PROMISE", taskName, timer, vibe, category, playlistId, spotifyId);
        await Promise.all (
            await Task.upsert({
                task_name: taskName,
                timer: timer,
